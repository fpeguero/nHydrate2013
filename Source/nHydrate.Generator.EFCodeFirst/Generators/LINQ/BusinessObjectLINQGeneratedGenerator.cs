...
{
	[GeneratorItemAttribute("LINQGenerated", typeof(LINQExtenderGenerator))]
	class LINQGeneratedGenerator : EFCodeFirstProjectItemGenerator
	{
		#region Class Members

		private const string RELATIVE_OUTPUT_LOCATION = @"\LINQ\";
		...
		public override void Generate()
		{
			foreach (Table table in _model.Database.Tables.Where(x => x.Generated).OrderBy(x => x.Name))
			{
				if (table.Generated)
				{
					LINQGeneratedTemplate template = new LINQGeneratedTemplate(_model, table);
