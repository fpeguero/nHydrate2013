...
{
	[GeneratorItemAttribute("IncludeTreeLINQExtenderGenerator", typeof(EFCodeFirstProjectGenerator))]
	class IncludeTreeLINQExtenderGenerator : EFCodeFirstProjectItemGenerator
	{
		#region Class Members

		private const string RELATIVE_OUTPUT_LOCATION = @"\ContextIncludeTree\";
		...
		public override void Generate()
		{
			foreach (Table table in _model.Database.Tables.Where(x => x.Generated).OrderBy(x => x.Name))
			{
				if (table.Generated)
				{
					IncludeTreeLINQExtenderTemplate template = new IncludeTreeLINQExtenderTemplate(_model, table);
