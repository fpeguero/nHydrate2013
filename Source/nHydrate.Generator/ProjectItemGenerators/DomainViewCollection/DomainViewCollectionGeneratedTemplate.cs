...
namespace Widgetsphere.Generator.ProjectItemGenerators.DomainViewCollection
{
	class DomainViewCollectionGeneratedTemplate : BaseClassTemplate
	{

		private StringBuilder sb = new StringBuilder();
		private CustomView _currentView;

		public DomainViewCollectionGeneratedTemplate(ModelRoot model, CustomView currentView)
		{
			_model = model;
		...
		public override string FileName
		{
			get
			{
				return string.Format("Domain{0}Collection.Generated.cs", _currentView.PascalName);
			}
		}
		...
		public string ParentItemName
		{
			get
			{
				return string.Format("Domain{0}Collection.cs", _currentView.PascalName);
			}
		}
		...
		private void AppendClass()
		{
			...
			sb.AppendLine("		/// This is an customizable extender for the domain class associated with the '" + _currentView.PascalName + "' object collection");
			sb.AppendLine("		/// </summary>");
			sb.AppendLine("	[Serializable()]");
			sb.AppendLine("	[System.ComponentModel.DesignerCategoryAttribute(\"code\")]");
			sb.AppendLine("	partial class Domain" + _currentView.PascalName + "Collection : ReadOnlyDomainCollection, IEnumerable, ISerializable");
			sb.AppendLine("	{");
		...
		private void AppendRegionSerializable()
		{
			...
			sb.AppendLine("		/// </summary>");
			sb.AppendLine("		protected Domain" + _currentView.PascalName + "Collection(SerializationInfo info, StreamingContext context):this()");
			sb.AppendLine("		{");
			sb.AppendLine();
			sb.AppendLine("		}");
			sb.AppendLine();
		...
		private void AppendRegionSelectMethods()
		{
			...
			//Search RunSelect
			sb.AppendLine("		internal static Domain" + _currentView.PascalName + "Collection RunSelect(string modifier)");
			sb.AppendLine("		{");
			sb.AppendLine("			SubDomain sd = new SubDomain(modifier);");
			sb.AppendLine("			sd.AddSelectCommand(new " + _currentView.PascalName + "SelectAll());");
			sb.AppendLine("			sd.RunSelectCommands();");
			sb.AppendLine("			return (Domain" + _currentView.PascalName + "Collection)sd.GetDomainCollection(Collections." + _currentView.PascalName + "Collection);");
			sb.AppendLine("		}");
			sb.AppendLine();
			sb.AppendLine("		internal static Domain" + _currentView.PascalName + "Collection RunSelect(" + _currentView.PascalName + "Search search, string modifier)");
			sb.AppendLine("		{");
			sb.AppendLine("			SubDomain sd = new SubDomain(modifier);");
			sb.AppendLine("			sd.AddSelectCommand(new " + _currentView.PascalName + "SelectBySearch(search));");
			sb.AppendLine("			sd.RunSelectCommands();");
			sb.AppendLine("			return (Domain" + _currentView.PascalName + "Collection)sd.GetDomainCollection(Collections." + _currentView.PascalName + "Collection);");
			sb.AppendLine("		}");
			sb.AppendLine();
			sb.AppendLine("		internal static Domain" + _currentView.PascalName + "Collection RunSelect(int page, int pageSize, string orderByColumn, bool ascending, string filter, out int count, string modifier)");
			sb.AppendLine("		{");
			sb.AppendLine("			Domain" + _currentView.PascalName + "Collection returnVal = null;");
			sb.AppendLine("			SubDomain sd = new SubDomain(modifier);");
			sb.AppendLine("			" + _currentView.PascalName + "PagedSelect retrieveRule = new " + _currentView.PascalName + "PagedSelect(page, pageSize, orderByColumn, ascending, filter);");
			sb.AppendLine("			sd.AddSelectCommand(retrieveRule);");
			sb.AppendLine("			sd.RunSelectCommands();");
			sb.AppendLine("			count = retrieveRule.Count;");
			sb.AppendLine("			returnVal = (Domain" + _currentView.PascalName + "Collection)sd.GetDomainCollection(Collections." + _currentView.PascalName + "Collection);");
			...
			{
				sb.Append("		internal static Domain" + _currentView.PascalName + "Collection RunSelect(");
				...
				sb.AppendLine("			Domain" + _currentView.PascalName + "Collection returnVal = null;");
				sb.AppendLine("			SubDomain sd = new SubDomain(modifier);");
				sb.AppendLine("			" + _currentView.PascalName + "Select retrieveRule = new " + _currentView.PascalName + "Select(" + GetParameterList(false) + ");");
				sb.AppendLine("			sd.AddSelectCommand(retrieveRule);");
				sb.AppendLine("			sd.RunSelectCommands();");
				sb.AppendLine("		 returnVal = (Domain" + _currentView.PascalName + "Collection)sd.GetDomainCollection(Collections." + _currentView.PascalName + "Collection);");
				sb.AppendLine("		 return returnVal;");
		...
		{
			sb.AppendLine("		#region Domain" + _currentView.PascalName + " Collection Methods");
			sb.AppendLine();
			sb.AppendLine("		internal void Remove" + _currentView.PascalName + "(Domain" + _currentView.PascalName + " element) ");
			sb.AppendLine("		{");
			...
			sb.AppendLine("		internal Domain" + _currentView.PascalName + " this[int index]");
			sb.AppendLine("		{");
			sb.AppendLine("			get { return ((Domain" + _currentView.PascalName + ")(base.Rows[index])); }");
			sb.AppendLine("		}");
			sb.AppendLine();
			sb.AppendLine("		internal DataView GetSorted" + _currentView.PascalName + "List(string sortString)");
			sb.AppendLine("		{");
		...
		private void AppendRegionProtectedOverrides()
		{
			...
			sb.AppendLine("		{");
			sb.AppendLine("			return new Domain" + _currentView.PascalName + "Collection();");
			sb.AppendLine("		}");
			sb.AppendLine();
			sb.AppendLine("		/// <summary>");
			...
			sb.AppendLine("		/// <returns>A new datarow of type '" + _currentView.PascalName + "'</returns>");
			sb.AppendLine("		protected override DataRow NewRowFromBuilder(DataRowBuilder builder) ");
			sb.AppendLine("		{");
			sb.AppendLine("			return new Domain" + _currentView.PascalName + "(builder);");
			sb.AppendLine("		}");
			sb.AppendLine();
			sb.AppendLine("		/// <summary>");
			...
			sb.AppendLine("		{");
			sb.AppendLine("			return typeof(Domain" + _currentView.PascalName + ");");
			sb.AppendLine("		}");
			sb.AppendLine();
			sb.AppendLine("		#endregion");
			sb.AppendLine();
