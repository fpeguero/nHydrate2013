...
{
	[GeneratorItemAttribute("DomainComponentCollectionGenerated", typeof(DomainComponentCollectionExtenderGenerator))]
	class DomainComponentCollectionGeneratedGenerator : BaseClassGenerator
	{
		#region Class Members

		private const string RELATIVE_OUTPUT_LOCATION = @"\Domain\Objects\Components\";
    ...
    public override void Generate()
		{
			foreach (Table table in (from x in _model.Database.Tables where x.Generated orderby x.Name select x))
			{
				if (table.Generated)
				{
					...
					{
						DomainComponentCollectionGeneratedTemplate template = new DomainComponentCollectionGeneratedTemplate(_model, component);
