...
{
	[GeneratorItemAttribute("DomainComponentCollectionExtender", typeof(DomainProjectGenerator))]
	class DomainComponentCollectionExtenderGenerator : BaseClassGenerator
	{
		...
		public override void Generate()
		{
			foreach (Table table in (from x in _model.Database.Tables where x.Generated orderby x.Name select x))
			{
				if (table.Generated)
				{
					...
					{
						DomainComponentCollectionExtenderTemplate template = new DomainComponentCollectionExtenderTemplate(_model, component);
