...
{
	[GeneratorItemAttribute("SelectComponentCommandGenerated", typeof(SelectComponentCommandExtenderGenerator))]
	class SelectComponentCommandGeneratedGenerator : BaseClassGenerator
	{
		#region Class Members

		private const string RELATIVE_OUTPUT_LOCATION = @"\Business\SelectCommands\";
      ...
      try
      {
        foreach(Table table in _model.Database.Tables)
        {
          if(table.Generated)
          {
						...
						{
							SelectComponentCommandGeneratedTemplate template = new SelectComponentCommandGeneratedTemplate(_model, component);
