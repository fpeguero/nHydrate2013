...
namespace Widgetsphere.Generator.ProjectItemGenerators.DomainStoredProcedure
{
  [GeneratorItemAttribute("DomainStoredProcedureGeneratedTemplate", typeof(DomainStoredProcedureExtenderGenerator))]
	class DomainStoredProcedureGeneratedGenerator : BaseClassGenerator
	{
		#region Class Members

    private const string RELATIVE_OUTPUT_LOCATION = @"\Domain\StoredProcedures\";
    ...
    public override void Generate()
      ...
      try
      {
        foreach (CustomStoredProcedure storedProcedure in _model.Database.CustomStoredProcedures.OrderBy(x => x.Name))
        {
          ...
          {
            DomainStoredProcedureGeneratedTemplate template = new DomainStoredProcedureGeneratedTemplate(_model, storedProcedure);
