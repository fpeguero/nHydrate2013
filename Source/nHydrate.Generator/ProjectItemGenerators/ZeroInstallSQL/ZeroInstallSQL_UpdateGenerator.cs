...
{
	[GeneratorItemAttribute("ZeroInstallSQLTemplate", typeof(DomainProjectGenerator))]
	class ZeroInstallSQL_UpdateGenerator : BaseClassGenerator
	{
		#region Class Members

		private const string PARENT_ITEM_NAME = @"Domain\SQLRaw\";

		internal enum ZeroInstallSQLTypeConstants
		{
			Delete,
			Insert,
			PagedSelect,
			//SearchSelectAnd, (Not Supported)
			//SearchSelectOr, (Not Supported)
			//Select, (handled with LINQ)
			//SelectByField, (handled with LINQ)
			SelectByForeignKey, 
			//SelectByPrimaryKey, (handled with LINQ)
			Update,
		}
      ...
      get { return _model.Database.Tables.Count; }
		...
		public override void Generate()
		{
			...
			foreach (Table table in (from x in _model.Database.Tables where x.Generated orderby x.Name select x))
			{
				if (table.Generated)
				{
					ZeroInstallSQL_UpdateTemplate template = new ZeroInstallSQL_UpdateTemplate(_model, table);
