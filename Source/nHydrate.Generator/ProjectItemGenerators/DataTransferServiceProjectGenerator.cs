...
{
	[GeneratorItemAttribute("Data Transfer Service (REST Services)", typeof(WidgetsphereGeneratorProject))]
	public class DataTransferServiceProjectGenerator : BaseProjectGenerator
	{
		...
		protected override string ProjectTemplate
		{
			get
			{
				GenerateCompanySpecificFile("datatransferservice.csproj");
				GenerateCompanySpecificFile("datatransferservice.vstemplate");
				return string.Format("{0}datatransferservice.vstemplate", ((ModelRoot)_model).CompanyName);
			}
		}
