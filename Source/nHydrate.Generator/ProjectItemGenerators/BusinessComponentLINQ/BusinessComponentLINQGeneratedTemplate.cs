...
namespace Widgetsphere.Generator.ProjectItemGenerators.BusinessComponentLINQ
{
	class BusinessComponentLINQGeneratedTemplate : BaseClassTemplate
	{
		private StringBuilder sb = new StringBuilder();
		private TableComponent _currentComponent;

		public BusinessComponentLINQGeneratedTemplate(ModelRoot model, TableComponent currentComponent)
		{
			_model = model;
		...
		private void AppendClass()
		{
			...
			{				
				...
				sb.AppendLine("	[Table(Name = \"" + _currentComponent.DatabaseName + "\")]");
				sb.AppendLine("	public partial class " + _currentComponent.PascalName + "Query");
				sb.AppendLine("	{");
				...
				foreach (Reference reference in _currentComponent.Columns)
				{
					...
					string description = column.Description.Trim();
					if (description != "") description += "\r\n		///";					
					description += "(Maps to the '" + _currentComponent.DatabaseName + "." + column.DatabaseName + "' database field)";					

					sb.AppendLine("		/// <summary>");
					sb.AppendLine("		/// " + description);
					sb.AppendLine("		/// </summary>");
					sb.Append("		[Column(");
					sb.Append("Name = \"" + column.DatabaseName + "\", ");
					sb.Append("DbType = \"" + column.DatabaseType + "\", ");
					sb.Append("CanBeNull = " + column.AllowNull.ToString().ToLower() + ", ");
					sb.Append("IsPrimaryKey = " + _currentComponent.Parent.PrimaryKeyColumns.Contains(column).ToString().ToLower());
					sb.AppendLine(")]");
					sb.AppendLine("		public " + column.GetCodeType(true) + " " + column.PascalName + " { get; set; }");
				}
				...
				if (_currentComponent.Parent.AllowCreateAudit)
				{
					...
					sb.AppendLine("		public DateTime? " + StringHelper.DatabaseNameToPascalCase(_model.Database.CreatedDateColumnName) + " { get; set; }");
					...
					sb.AppendLine("		public string " + StringHelper.DatabaseNameToPascalCase(_model.Database.CreatedByColumnName) + " { get; set; }");
				}
				...
				if (_currentComponent.Parent.AllowModifiedAudit)
				{
					...
					sb.AppendLine("		public DateTime? " + StringHelper.DatabaseNameToPascalCase(_model.Database.ModifiedDateColumnName) + " { get; set; }");
					...
					sb.AppendLine("		public string " + StringHelper.DatabaseNameToPascalCase(_model.Database.ModifiedByColumnName) + " { get; set; }");
				}
				...
				if (_currentComponent.Parent.AllowTimestamp)
				{
					...
					sb.AppendLine("		public byte[] " + StringHelper.DatabaseNameToPascalCase(_model.Database.TimestampColumnName) + " { get; set; }");
				}
				sb.AppendLine();

				//Add child relationships				
				foreach (Relation relation in _model.Database.Relations.FindByParentTable(_currentComponent.Parent, true).Where(x => x.IsGenerated))
				{
				...
				foreach (Relation relation in _model.Database.Relations.FindByChildTable(_currentComponent.Parent, true).Where(x => x.IsGenerated))
				{						
					Table parentTable = (Table)relation.ParentTableRef.Object;

					//Do not process self-referencing relationships
					if (parentTable != _currentComponent.Parent)
					{
						string thisKey = "";
						string otherKey = "";
						foreach (ColumnRelationship columnRelationship in relation.ColumnRelationships)
