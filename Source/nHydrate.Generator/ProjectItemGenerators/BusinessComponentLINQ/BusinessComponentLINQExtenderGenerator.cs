...
{
	[GeneratorItemAttribute("BusinessComponentLINQExtender", typeof(DomainProjectGenerator))]
	class BusinessComponentLINQExtenderGenerator : BaseClassGenerator
	{
		#region Class Members

		private const string RELATIVE_OUTPUT_LOCATION = @"\Business\LINQ\";
		...
		public override void Generate()
		{
			foreach (Table table in (from x in _model.Database.Tables where x.Generated orderby x.Name select x))
			{
				if (table.Generated)
				{
					...
					{
						BusinessComponentLINQExtenderTemplate template = new BusinessComponentLINQExtenderTemplate(_model, component);
