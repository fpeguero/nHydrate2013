...
{
	[GeneratorItemAttribute("BusinessComponentCollectionGenerated", typeof(BusinessComponentCollectionExtenderGenerator))]
	class BusinessComponentCollectionGeneratedGenerator : BaseClassGenerator
	{
		#region Class Members

		private const string RELATIVE_OUTPUT_LOCATION = @"\Business\Objects\Components\";
    ...
    public override void Generate()
		{
			foreach (Table table in (from x in _model.Database.Tables where x.Generated orderby x.Name select x))
			{
				if (table.Generated)
				{
					...
					{
						BusinessComponentCollectionGeneratedTemplate template = new BusinessComponentCollectionGeneratedTemplate(_model, component);
