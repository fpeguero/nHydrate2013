...
{
	[GeneratorItemAttribute("DataServiceJScriptsGenerator", typeof(DataTransferServiceProjectGenerator))]
	class DataServiceJScriptsGenerator : BaseGenerator
	{
		#region Class Members

		private const string RELATIVE_OUTPUT_LOCATION = @"\scripts\generated\objects\";
      ...
      get { return _model.Database.Tables.Count; }
		...
		public override void Generate()
		{
			foreach (Table table in (from x in _model.Database.Tables where x.Generated orderby x.Name select x))
			{
				if (table.Generated)
				{
					DataServiceJScriptsTemplate template = new DataServiceJScriptsTemplate(_model, table);
