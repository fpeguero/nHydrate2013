...
namespace Widgetsphere.Generator.ProjectItemGenerators.DataService.DataServiceObjectGridJS
{
	class ObjectGridJSTemplate : BaseDataTransferServiceTemplate
	{
		...
		{
			get { return _currentTable.PascalName.ToLower() + "grid.js"; }
		}
		...
		private void GenerateContent()
		{
			try
			{
				List<Column> columnList =(from x in _currentTable.GetColumnsFullHierarchy()
														where !x.IsBinaryType
														select x).ToList();
				...
				sb.AppendLine("    caption: \"" + _currentTable.PrimaryKeyColumns[0].PascalName + "\",");
				sb.AppendLine("    gridview: true,");
				sb.AppendLine("    rownumbers: true,");
				sb.AppendLine("    rownumWidth: 40,");
				sb.AppendLine("    scroll: 1,");
  				sb.AppendLine("    rowNum: 100,");
				...
				sb.AppendLine("    var grid = jQuery(\"#grid\").jqGrid(this.gridConfiguration);");
				sb.AppendLine("    grid.setGridHeight(outerLayout.panes.center.innerHeight() - 105);");
				sb.AppendLine("};");
			}
			catch (Exception ex)
			{
