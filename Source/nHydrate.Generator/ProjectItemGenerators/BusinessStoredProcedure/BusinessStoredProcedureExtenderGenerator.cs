...
{
	[GeneratorItemAttribute("BusinessStoredProcedureExtender", typeof(DomainProjectGenerator))]
	class BusinessStoredProcedureExtenderGenerator : BaseClassGenerator
	{
		#region Class Members

    private const string RELATIVE_OUTPUT_LOCATION = @"\Business\StoredProcedures\";
      ...
      try
      {
				foreach (CustomStoredProcedure storedProcedure in _model.Database.CustomStoredProcedures.OrderBy(x => x.Name))
        {
          if(storedProcedure.Generated)
          {
            ...
            BusinessStoredProcedureExtenderTemplate template = new BusinessStoredProcedureExtenderTemplate(_model, storedProcedure);
