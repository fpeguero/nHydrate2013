...
{
  [GeneratorItemAttribute("DefinedSelectCommandGenerated", typeof(DefinedSelectCommandExtenderGenerator))]
  class DefinedSelectCommandGeneratedGenerator : BaseClassGenerator
  {
    #region Class Members

    private const string RELATIVE_OUTPUT_LOCATION = @"\Business\DefinedSelectCommands\";
    ...
    {
      get { return _model.Database.CustomRetrieveRules.Count; }
    }
    ...
    public override void Generate()
    {
      ...
      try
      {
        foreach(CustomRetrieveRule rule in _model.Database.CustomRetrieveRules)
        {
          if(rule.Generated)
          {
            DefinedSelectCommandGeneratedTemplate template = new DefinedSelectCommandGeneratedTemplate(_model, rule);
