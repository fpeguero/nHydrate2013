...
{
	class DefinedSelectCommandExtenderTemplate : BaseClassTemplate
	{
		...
		public DefinedSelectCommandExtenderTemplate(ModelRoot model, CustomRetrieveRule currentRule)
    {
      ...
      this.ParentTable = (Table)currentRule.ParentTableRef.Object;
		...
		public override string FileName
		{
			get 
			{
        return string.Format("{0}SelectBy{1}.cs", this.ParentTable.PascalName, _currentRule.PascalName);
			}
		}
		#endregion
      ...
      try
      {
        sb.AppendLine("namespace " + DefaultNamespace + ".Business.DefinedSelectCommands" );
        sb.AppendLine("{" );
        sb.AppendLine("	class " + this.ParentTable.PascalName + "SelectBy" + _currentRule.PascalName );
        sb.AppendLine("	{" );
        sb.AppendLine("	}" );
        sb.AppendLine("}" );
      }
