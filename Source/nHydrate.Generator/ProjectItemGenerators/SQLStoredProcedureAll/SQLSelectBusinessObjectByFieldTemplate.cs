...
{
	class SQLSelectBusinessObjectByFieldTemplate : ISQLGenerate
	{
		private ModelRoot _model;
		private Table _currentTable;

		#region Constructors
		public SQLSelectBusinessObjectByFieldTemplate(ModelRoot model, Table currentTable)
		{
			_model = model;
		...
		protected string BuildSelectList()
		{
			...
			foreach (Column dc in _currentTable.GetColumnsFullHierarchy())
			{
				...
				if (ii != _currentTable.GeneratedColumns.Count) output.Append(",");
				output.AppendLine();
				ii++;
			}
