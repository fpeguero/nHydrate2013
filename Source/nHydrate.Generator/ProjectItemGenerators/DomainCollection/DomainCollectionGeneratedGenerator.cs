...
{
	[GeneratorItemAttribute("DomainCollectionGenerated", typeof(DomainCollectionExtenderGenerator))]
	class DomainCollectionGeneratedGenerator : BaseClassGenerator
	{
    ...
    public override void Generate()
		{
			foreach (Table table in (from x in _model.Database.Tables where x.Generated orderby x.Name select x))
			{
				if (table.Generated)
				{
					DomainCollectionGeneratedTemplate template = new DomainCollectionGeneratedTemplate(_model, table);
