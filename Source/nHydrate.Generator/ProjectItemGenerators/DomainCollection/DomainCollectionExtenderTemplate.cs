...
{
	class DomainCollectionExtenderTemplate : BaseClassTemplate
	{
		private StringBuilder sb = new StringBuilder();
		private Table _currentTable;

		#region Constructors
		public DomainCollectionExtenderTemplate(ModelRoot model, Table currentTable)
    {
      _model = model;
      _currentTable = currentTable;
		...
		public override string FileName
		{
			get 
			{
				return string.Format("Domain{0}Collection.cs", _currentTable.PascalName);
			}
		}
		#endregion
      ...
      catch(Exception ex)
      {
		...
		{
      sb.AppendLine("	internal partial class " + "Domain" + _currentTable.PascalName + "Collection");
			sb.AppendLine("	{");
			sb.AppendLine("	}");
		}
		#endregion
