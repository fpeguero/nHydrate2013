...
{
	class BusinessViewCollectionExtenderTemplate : BaseClassTemplate
	{
		private StringBuilder sb = new StringBuilder();
    private CustomView _currentView;

		#region Constructors
    public BusinessViewCollectionExtenderTemplate(ModelRoot model, CustomView currentView)
    {
      _model = model;
      _currentView = currentView;
		...
		public override string FileName
		{
			get 
			{
        return string.Format("{0}Collection.cs", _currentView.PascalName);
			}
		}
		#endregion
      ...
      catch(Exception ex)
      {
		...
		{
      sb.AppendLine("	partial class " + _currentView.PascalName + "Collection");
			sb.AppendLine("	{");
			sb.AppendLine("	}");
		}
		#endregion
