...
{
	[GeneratorItemAttribute("BusinessCollectionGenerated", typeof(BusinessCollectionExtenderGenerator))]
	class BusinessCollectionGeneratedGenerator : BaseClassGenerator
	{

		#region Class Members

		private const string RELATIVE_OUTPUT_LOCATION = @"\Business\Objects\";
    ...
    public override void Generate()
		{
			foreach (Table table in (from x in _model.Database.Tables where x.Generated orderby x.Name select x))
			{
				if (table.Generated)
				{
					BusinessCollectionGeneratedTemplate template = new BusinessCollectionGeneratedTemplate(_model, table);
