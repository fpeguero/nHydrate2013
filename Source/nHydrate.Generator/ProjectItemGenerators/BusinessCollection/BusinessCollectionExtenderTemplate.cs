...
{
	class BusinessCollectionExtenderTemplate : BaseClassTemplate
	{
		private StringBuilder sb = new StringBuilder();
		private Table _currentTable;

		#region Constructors
		public BusinessCollectionExtenderTemplate(ModelRoot model, Table currentTable)
    {
      _model = model;
      _currentTable = currentTable;
		...
		public override string FileName
		{
			get 
			{
				return string.Format("{0}Collection.cs", _currentTable.PascalName);
			}
		}
		#endregion
      ...
      catch(Exception ex)
      {
		...
		{
      sb.AppendLine("	partial class " + _currentTable.PascalName + "Collection");
			sb.AppendLine("	{");
			sb.AppendLine("	}");
		}
