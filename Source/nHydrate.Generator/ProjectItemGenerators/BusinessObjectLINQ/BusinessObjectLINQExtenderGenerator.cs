...
{
	[GeneratorItemAttribute("BusinessObjectLINQExtender", typeof(DomainProjectGenerator))]
	class BusinessObjectLINQExtenderGenerator : BaseClassGenerator
	{
    ...
    public override void Generate()
		{
			foreach (Table table in (from x in _model.Database.Tables where x.Generated orderby x.Name select x))
			{
				if (table.Generated)
				{
					BusinessObjectLINQExtenderTemplate template = new BusinessObjectLINQExtenderTemplate(_model, table);
