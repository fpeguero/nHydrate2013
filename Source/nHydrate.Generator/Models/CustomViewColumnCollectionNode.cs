...
{
	public class CustomViewColumnCollectionNode : ModelObjectTreeNode
	{
		...
		public CustomViewColumnCollectionNode(CustomViewColumnCollectionController controller)
			: base(controller)
		{
		}
		...
		public override void Refresh()
		{
			try
			{
				...
				if((this.TreeView != null) && (this.TreeView.SelectedNode != null) && (this.Nodes.Contains(this.TreeView.SelectedNode)))
				{
					...
					if(reference != null)
						selectedKey = ((CustomViewColumn)reference.Object).Key;
				}
				...
				foreach(Reference reference in referenceCollection)
				{
					...
					{
						var tc = new CustomViewColumnController(reference.Object);
						tc.Node.Name = reference.Key;
						tc.Node.Text = ((CustomViewColumn)reference.Object).Name;
				...
				foreach (TreeNode node in this.Nodes)
				{
					...
					if (item != null) node.Text = ((CustomViewColumn)item.Object).Name;
				}

				//Remove non-existing nodes
				for(var ii = this.Nodes.Count - 1 ; ii >= 0 ; ii--)
				{
					var node = (CustomViewColumnNode)this.Nodes[ii];
				...
				if((this.TreeView != null) && (!string.IsNullOrEmpty(selectedKey)))
				{
					foreach(TreeNode node in this.Nodes)
					{
						...
						if((reference != null) && (selectedKey == ((CustomViewColumn)reference.Object).Key))
