...
{
	public partial class TableComponentColumnForm : Form
	{
		#region Class Members

		private readonly TableComponent _tablecomponent = null;

		#endregion

		#region Constructors

		public TableComponentColumnForm()
		{
			InitializeComponent();
		}
		...
		{
			var columnList = _tablecomponent.Parent.GetColumnsFullHierarchy();
			foreach (var column in (from x in columnList orderby x.Name select x))
			{
				lstMembers.Items.Add(column);
			}

			foreach (Reference reference in _tablecomponent.Columns)
			{
		...
		{
			_tablecomponent.Columns.Clear();
			foreach (Column column in lstMembers.CheckedItems)
			{
				_tablecomponent.Columns.Add(column.CreateRef());
			}
