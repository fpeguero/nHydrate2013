...
{
	public class DaatTransferStoredProcedureGeneratedTemplate : BaseDataTransferTemplate
	{
		private StringBuilder sb = new StringBuilder();
		private CustomStoredProcedure _currentView;

		public DaatTransferStoredProcedureGeneratedTemplate(ModelRoot model, CustomStoredProcedure currentView)
		{
			_model = model;
		...
		private void AppendClass()
		{
			try
			{
				...
				List<CustomStoredProcedureColumn> imageColumnList = _currentView.GetColumnsByType(System.Data.SqlDbType.Image);
				if (imageColumnList.Count != 0)
				{
					...
					sb.AppendLine("		{");
					foreach (CustomStoredProcedureColumn column in imageColumnList)
					{
						sb.AppendLine("			 /// <summary>");
				...
				sb.AppendLine("		{");
				foreach (CustomStoredProcedureColumn column in _currentView.GetColumns().Where(x => x.Generated))
				{
					sb.AppendLine("			 /// <summary>");
				...
				foreach (CustomStoredProcedureColumn column in _currentView.GeneratedColumns)
				{					
					sb.AppendLine("		/// <summary>");
