...
{
	class ObjectListPagesExtenderTemplate : ObjectListPagesTemplate
	{
		private StringBuilder sb = new StringBuilder();
		private Table _currentTable;

		#region Constructors
		public ObjectListPagesExtenderTemplate(ModelRoot model, Table currentTable)
    {
      _model = model;
      _currentTable = currentTable;
		...
		public override string FileName
		{
			get 
			{
				return string.Format("{0}.cs", _currentTable.PascalName);
			}
		}
		#endregion
      ...
      catch(Exception ex)
      {
		...
		{
			sb.AppendLine("	partial class " + _currentTable.PascalName + "Extension");
			sb.AppendLine("	{");
			sb.AppendLine("	}");
		}
