...
namespace PROJECTNAMESPACE
{
	internal class AzureCopy
	{
		public void Run(InstallSettings settings)
		{
			...
			var scripts = SqlServers.ReadSQLFileSectionsFromResource(resourceName);

			MySqlConnection connection = null;
			try
			{
				connection = new MySqlConnection(settings.GetCloudConnectionString());
				connection.Open();
			...
			//4. Run full installer on the target database
			UpgradeInstaller.UpgradeDatabase(settings.GetCloudConnectionString(), false, null);
		...
		private void CopyData(InstallSettings settings)
		{
			...
			try
			{
				connection = new MySqlConnection(settings.GetPrimaryConnectionString());
				connection.Open();
				var adpater = new MySqlDataAdapter("select T.name as tablename, S.name as schemaname from sys.tables T inner join sys.schemas S on T.schema_id = S.schema_id where T.name <> 'dtproperties' AND T.name <> 'sysdiagrams' order by tablename", connection);
		...
		{
			var connection = new MySqlConnection(settings.GetCloudConnectionString());
			var adapter = new MySqlDataAdapter("select * from sys.tables", connection);
			var ds = new DataSet();
			adapter.Fill(ds);
		...
		private string GetColumnList(string connectionString, string tableName)
		{
			...
			try
			{
				...
				connection.Open();
				var adpater = new MySqlDataAdapter("select C.name as columnname from sys.columns C inner join sys.objects O ON C.object_id = O.object_id where O.name = '" + tableName + "' order by C.column_id", connection);
				DataSet ds = new DataSet();
				adpater.Fill(ds);
